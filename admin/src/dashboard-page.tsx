...
export function DashboardPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-5 xl:grid-cols-3">
      <Card><CardHeader><CardTitle>运营待办</CardTitle><CardDescription>优先关注需要处理的运行状态</CardDescription></CardHeader><CardContent className="space-y-4">{platform ? <><div className="admin-attention"><HeartPulseIcon className="size-5" /><div className="flex-1"><p className="font-medium">失败工作项</p><p className="text-xs text-muted-foreground">Agent、知识处理与通知合计</p></div><strong className="text-2xl tabular-nums">{platform.counts.failedJobs}</strong></div><div className="flex flex-wrap gap-2"><Button asChild variant="outline" size="sm"><Link to="/resources/agent-work-items">Agent 工作项</Link></Button><Button asChild variant="outline" size="sm"><Link to="/resources/knowledge-jobs">知识任务</Link></Button><Button asChild variant="outline" size="sm"><Link to="/resources/notification-deliveries">通知投递</Link></Button></div><div className="space-y-3 border-t pt-4">{Object.entries(platform.dependencies).map(([key, ready]) => <div key={key} className="flex items-center justify-between gap-3 text-sm"><span>{key}</span><StatusBadge value={ready ? 'ready' : 'unhealthy'} /></div>)}</div></> : <p className="text-sm text-muted-foreground">等待业务概览数据</p>}</CardContent></Card>
