...
export async function searchResearch(query: string, limit = 8, signal?: AbortSignal): Promise<{ provider: 'OpenAlex'; query: string; results: ResearchSearchResult[] }> {
  ...
  const { body } = await fetchResearch(endpoint, signal)
