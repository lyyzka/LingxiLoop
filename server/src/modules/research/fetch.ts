...
export function pinnedRequestOptions(url: URL, address: string): RequestOptions {
  ...
  return { hostname: address, agent: false,
    ...
    headers: { host: url.host, accept: 'text/html,application/json,text/plain', 'accept-encoding': 'identity', 'user-agent': 'LingxiLoop-Research/1.0' },
  }
}

export async function fetchResearch(raw: string, signal?: AbortSignal): Promise<{ url: string; contentType: string; body: Buffer }> {
  signal?.throwIfAborted()
  const controller = new AbortController()
  let timeout: ReturnType<typeof setTimeout> | undefined
  let abort: (() => void) | undefined
  const expired = new Promise<never>((_resolve, reject) => {
    abort = () => { controller.abort(signal?.reason); reject(signal?.reason ?? new Error('research cancelled')) }
