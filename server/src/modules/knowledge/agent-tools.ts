...
import { nativeTool, nativeContext, compareResource } from '../../agents/tools.js'
...
async function authorize(context: ActionContext, input: Record<string, unknown> = {}) {
  const projectId = await findKnowledgeRetrievalProject(db(context), context.work.tenantId, context.work.sessionId, context.work.principalId!)
...
export const knowledgeTools: ToolDefinition[] = [
  nativeTool('knowledge.list_sources', schemas.list_sources, { description: 'Read sources visible to the original human in the current workspace.', effect: 'read', approval: false, authorize,
    async execute(context) { return { ok: true, value: await application(context).listKnowledgeSourcesForAgent(nativeContext(context)) } } }),
  ...
  nativeTool('knowledge.add_file', schemas.add_file, { ...transaction, description: 'Use a committed attachment as a knowledge source.', async execute(context, input) {
    const messages = await readAgentChannelMessages({ companyId: context.work.tenantId, agentId: context.work.agentId, channelId: context.work.sessionId,
