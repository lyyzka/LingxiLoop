-- Install the pinned LingxiOS schema before any application process starts.
-- Source: lingxios package schema version 4.
...
-- Product ingress outbox: the WuKong receipt and wake intent commit together;
-- receive() runs only after commit and retries converge on LingxiOS work IDs.
CREATE TABLE public.lingxios_ingress_outbox (
  event_id text NOT NULL,
  agent_id text NOT NULL,
  company_id text NOT NULL,
  channel_id text NOT NULL,
  client_msg_no text NOT NULL,
  kind text NOT NULL CHECK (kind IN ('message','handoff','calendar')),
  attachment_client_msg_nos text[] NOT NULL DEFAULT '{}',
  ...
  FOREIGN KEY (agent_id,company_id) REFERENCES public.participants(id,company_id) ON DELETE CASCADE,
  FOREIGN KEY (knowledge_source_id) REFERENCES public.knowledge_sources(id) ON DELETE SET NULL
);
CREATE INDEX lingxios_ingress_claim_idx ON public.lingxios_ingress_outbox(available_at,created_at)
  WHERE delivered_at IS NULL AND available_at IS NOT NULL;

-- Preserve legacy approval audit identity without importing the retired work queue.
ALTER TABLE public.approvals ADD COLUMN legacy_work_id text;
ALTER TABLE public.approvals DROP CONSTRAINT approvals_work_id_fkey;
ALTER TABLE public.approvals DROP CONSTRAINT approvals_source_fields_check;
UPDATE public.approvals
   SET legacy_work_id=work_id, work_id=NULL
 WHERE work_id IS NOT NULL;
ALTER TABLE public.approvals ADD CONSTRAINT approvals_source_fields_check CHECK (
  (source='AGENT_OS' AND authorization_user_id IS NOT NULL AND idempotency_key IS NOT NULL
    AND action IS NOT NULL AND expires_at IS NOT NULL
    AND ((work_id IS NOT NULL AND legacy_work_id IS NULL)
      OR (work_id IS NULL AND legacy_work_id IS NOT NULL AND status IN ('EXECUTED','REJECTED','CANCELLED'))))
  OR (source='COWORKER' AND payload_hash IS NOT NULL AND kind IS NOT NULL)
);
ALTER TABLE public.approvals ADD CONSTRAINT approvals_work_id_fkey
  FOREIGN KEY (work_id) REFERENCES lingxios.agent_work_items(id) ON DELETE CASCADE;
CREATE INDEX approvals_legacy_work_idx ON public.approvals(legacy_work_id)
  WHERE legacy_work_id IS NOT NULL;
